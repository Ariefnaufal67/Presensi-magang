...
const RADIUS_DEFAULT_METER = 50;
// Akurasi GPS (radius error, dalam meter) yang dilaporkan browser lewat
// position.coords.accuracy. Kalau lebih besar dari ini, pembacaan GPS
// dianggap terlalu kasar untuk dipakai memutuskan presensi mandiri —
// peserta diminta coba lagi (biasanya cukup pindah ke tempat terbuka atau
// menunggu beberapa detik sampai sinyal GPS mengunci).
const AKURASI_MAKS_METER = 100;
let ensured = false;

function buatQrToken() {
  return crypto.randomBytes(6).toString('hex').toUpperCase();
}

// Jarak antara dua koordinat (derajat) dalam meter, pakai rumus Haversine.
...
async function ensureReady(db) {
  ...
  if (!existing) {
    await settingsCol.insertOne({
      ...
      lokasi: { lat: null, lng: null, radiusMeter: RADIUS_DEFAULT_METER },
      qrToken: buatQrToken()
    });
    ...
    // Migrasi dari versi lama tanpa fitur presensi mandiri (lokasi/qrToken
    // belum ada di dokumen settings) — isi sekali dengan nilai default.
    const patchLokasi = {};
    if (!existing.lokasi) {
      patchLokasi.lokasi = { lat: null, lng: null, radiusMeter: RADIUS_DEFAULT_METER };
    }
    if (!existing.qrToken) {
      patchLokasi.qrToken = buatQrToken();
    }
    if (Object.keys(patchLokasi).length > 0) {
      await settingsCol.updateOne({ _id: SETTINGS_ID }, { $set: patchLokasi });
    }
  }
...
async function getSettings() {
  ...
  return {
    ...
    lokasi: doc.lokasi || { lat: null, lng: null, radiusMeter: RADIUS_DEFAULT_METER },
    qrToken: doc.qrToken
  };
}

async function updateSettings({ jamMasuk, toleransi, lokasiLat, lokasiLng, radiusMeter }) {
  const db = await getReadyDb();
  const set = {};
  ...
  if (lokasiLat !== undefined && lokasiLat !== null && lokasiLat !== '') {
    const lat = parseFloat(lokasiLat);
    if (Number.isFinite(lat)) set['lokasi.lat'] = lat;
  }
  if (lokasiLng !== undefined && lokasiLng !== null && lokasiLng !== '') {
    const lng = parseFloat(lokasiLng);
    if (Number.isFinite(lng)) set['lokasi.lng'] = lng;
  }
  if (radiusMeter !== undefined && radiusMeter !== null && radiusMeter !== '') {
    const r = parseInt(radiusMeter, 10);
    if (Number.isFinite(r) && r > 0) set['lokasi.radiusMeter'] = r;
  }
...
// Buat ulang kode QR presensi mandiri yang ditampilkan di kiosk — dipakai
// kalau kode lama diduga bocor/tersebar (mis. discreenshot lalu dikirim ke
// orang yang tidak sedang berada di lokasi).
async function regenerateQrToken() {
  const db = await getReadyDb();
  const qrToken = buatQrToken();
  await db.collection('settings').updateOne({ _id: SETTINGS_ID }, { $set: { qrToken } });
  return qrToken;
}
...
// Logika inti pencatatan kehadiran (dipakai baik oleh scan yang dioperasikan
// admin di kiosk maupun presensi mandiri lewat HP peserta setelah lolos
// validasi lokasi). Tidak melakukan pengecekan apa pun selain status
// masuk/pulang — validasi lain (lokasi, token QR, dsb) dilakukan oleh
// pemanggil sebelum fungsi ini dipanggil.
async function recordAttendance(pesertaId) {
  const db = await getReadyDb();
...
// Scan yang dioperasikan admin di layar kiosk (kartu QR peserta dipindai
// pakai kamera kiosk, atau ID diketik manual oleh admin). Admin hadir
// secara fisik di lokasi, jadi tidak perlu validasi GPS.
async function processScan(pesertaId) {
  return recordAttendance(pesertaId);
}

// Presensi mandiri: peserta login sendiri lewat HP, memindai kode QR yang
// tertampil di layar kiosk, lalu HP-nya mengirim koordinat GPS. Di sinilah
// validasi token QR + jarak + akurasi GPS dilakukan sebelum kehadiran
// benar-benar dicatat.
async function processScanMandiri({ pesertaId, token, lat, lng, accuracy }) {
  const peserta = await findPesertaById(pesertaId);
  if (!peserta) return { ok: false, message: 'Peserta tidak ditemukan. Silakan login ulang.' };

  const settings = await getSettings();

  if (!token || token !== settings.qrToken) {
    return {
      ok: false,
      peserta,
      message: 'Kode QR tidak valid atau sudah kedaluwarsa. Pindai kode yang sedang tertampil di layar kiosk.'
    };
  }

  if (!settings.lokasi || settings.lokasi.lat === null || settings.lokasi.lng === null) {
    return {
      ok: false,
      peserta,
      message: 'Lokasi kantor/tempat magang belum diatur oleh admin. Hubungi admin untuk mengaturnya dulu.'
    };
  }
  ...
  if (!Number.isFinite(latNum) || !Number.isFinite(lngNum)) {
    return {
      ok: false,
      peserta,
      message: 'Lokasi HP tidak terbaca. Pastikan izin lokasi diaktifkan, lalu coba lagi.'
    };
  }

  if (!Number.isFinite(accNum) || accNum > AKURASI_MAKS_METER) {
    return {
      ok: false,
      peserta,
      message: `Akurasi GPS kurang baik (±${Number.isFinite(accNum) ? Math.round(accNum) : '?'}m). Coba lagi di tempat terbuka dengan sinyal GPS lebih baik.`
    };
  }

  const jarak = jarakMeter(latNum, lngNum, settings.lokasi.lat, settings.lokasi.lng);
  const radius = settings.lokasi.radiusMeter || RADIUS_DEFAULT_METER;
  if (jarak > radius) {
    return {
      ok: false,
      peserta,
      message: `Kamu berada di luar radius lokasi magang (jarak ±${Math.round(jarak)}m, maksimal ${radius}m). Presensi hanya bisa dilakukan dari lokasi magang.`
    };
  }
