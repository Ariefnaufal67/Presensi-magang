...
//                toleransi, dan lokasi kantor (officeLat/officeLng/officeRadius)
//                untuk verifikasi geofencing
//   admins       { username, passwordHash } — bisa lebih dari satu akun admin
//   session      satu dokumen tetap (_id: 'active_session') — token QR yang
//                ditampilkan di layar admin, berganti tiap ~20 detik
//   attendance   { pesertaId, nama, tanggal, jamMasuk, statusMasuk,
//                  jamPulang, pulangManual, lokasiMasuk } — satu dokumen per
//                  peserta per hari, unik lewat index {pesertaId, tanggal}
...
const SESSION_ID = 'active_session';
const SESSION_DURATION_MS = 60 * 60 * 1000; // QR di layar admin berganti tiap 1 jam
let ensured = false;
...
async function ensureReady(db) {
  ...
  if (!existing) {
    await settingsCol.insertOne({
      ...
      officeLat: seed.settings.officeLat,
      officeLng: seed.settings.officeLng,
      officeRadius: seed.settings.officeRadius
    });
    ...
    if (existing.adminUsername && existing.adminPasswordHash) {
    ...
    if (existing.officeLat === undefined || existing.officeLng === undefined) {
      // Migrasi dari versi sebelum fitur geofencing: isi default lokasi kantor.
      await settingsCol.updateOne(
        { _id: SETTINGS_ID },
        {
          $set: {
    ...
    if (existing.jamPulangOtomatis === undefined) {
      // Migrasi dari versi sebelum fitur pulang otomatis: isi default jam 17:00, aktif.
      await settingsCol.updateOne(
        { _id: SETTINGS_ID },
        {
          $set: {
            jamPulangOtomatis: seed.settings.jamPulangOtomatis,
            pulangOtomatisAktif: seed.settings.pulangOtomatisAktif
          }
        }
      );
    }
...
// Jarak antara dua koordinat GPS dalam meter (rumus Haversine).
...
async function getSettings() {
  ...
  return {
    ...
    toleransi: doc.toleransi,
    officeLat: doc.officeLat,
    officeLng: doc.officeLng,
    officeRadius: doc.officeRadius,
    jamPulangOtomatis: doc.jamPulangOtomatis || '17:00',
    pulangOtomatisAktif: doc.pulangOtomatisAktif !== undefined ? !!doc.pulangOtomatisAktif : true
  };
}
...
async function updateSettings({
  ...
  if (officeLat !== undefined && officeLat !== null && officeLat !== '') {
    set.officeLat = parseFloat(officeLat);
  }
  if (officeLng !== undefined && officeLng !== null && officeLng !== '') {
    set.officeLng = parseFloat(officeLng);
  }
  if (officeRadius !== undefined && officeRadius !== null && officeRadius !== '') {
    set.officeRadius = parseInt(officeRadius, 10) || 0;
  }
  if (jamPulangOtomatis) set.jamPulangOtomatis = jamPulangOtomatis;
  if (pulangOtomatisAktif !== undefined && pulangOtomatisAktif !== null) {
    set.pulangOtomatisAktif = !!pulangOtomatisAktif;
  }
  await db.collection('settings').updateOne({ _id: SETTINGS_ID }, { $set: set });
  return getSettings();
}

// Menandai pulang otomatis untuk semua peserta yang sudah absen masuk hari
// ini tapi belum tercatat pulang, dipanggil oleh Vercel Cron Job tiap hari
// pada jam yang dikonfigurasi (lihat handleAutoPulang di api/[...route].js).
// `force=true` (dipakai tombol "Jalankan Sekarang" admin) tetap berjalan
// meski toggle pulangOtomatisAktif sedang dimatikan, untuk keperluan tes.
async function runAutoCheckout({ force } = {}) {
  const db = await getReadyDb();
  const settingsDoc = await db.collection('settings').findOne({ _id: SETTINGS_ID });

  if (!force && !settingsDoc.pulangOtomatisAktif) {
    return { ok: true, dijalankan: false, message: 'Pulang otomatis sedang dimatikan di pengaturan.' };
  }

  const tanggal = todayKey();
  const waktu = formatJam(new Date());
  const attendanceCol = db.collection('attendance');
  const filter = { tanggal, sumber: 'scan', jamMasuk: { $ne: null }, jamPulang: null };
  const belumPulang = await attendanceCol.find(filter, { projection: { _id: 0 } }).toArray();

  if (belumPulang.length > 0) {
    await attendanceCol.updateMany(filter, {
      $set: { jamPulang: waktu, pulangManual: false, pulangOtomatis: true }
    });
  }
  ...
  return {
    ok: true,
    dijalankan: true,
    jumlah: belumPulang.length,
    waktu,
    peserta: belumPulang.map((e) => e.nama)
  };
}

// ---------- sesi QR (ditampilkan admin, di-scan peserta) ----------
...
// Ambil sesi QR yang sedang aktif; kalau belum ada atau sudah kedaluwarsa,
// buat token baru. Aman dipanggil berkali-kali (idempotent selama belum
// kedaluwarsa) — dipakai admin (polling untuk menampilkan QR + countdown)
// maupun saat validasi scan dari peserta.
async function getOrRefreshSession() {
  const db = await getReadyDb();
  const sessionCol = db.collection('session');
  const now = Date.now();
  const existing = await sessionCol.findOne({ _id: SESSION_ID });
  ...
  await sessionCol.updateOne(
    { _id: SESSION_ID },
    { $set: { token, expiresAt, tanggal: todayKey() } },
    { upsert: true }
  );
...
async function findPeserta(needle) {
  ...
  const docs = await db.collection('roster').find({}, { projection: { _id: 0 } }).toArray();
  return docs.find((x) => x.id.toLowerCase() === n) || null;
}
...
const MAKS_AKURASI_GPS = 5000; // meter — cuma sanity check ekstrem (menyaring bacaan yang benar-benar rusak/tidak masuk akal).
// Catatan: di dalam gedung, sinyal GPS satelit sering terhalang beton/atap
// sehingga HP jatuh ke triangulasi WiFi/seluler yang akurasinya jauh lebih
// kasar (ratusan meter, kadang 1000-2000m+). Ambang ini sengaja dibuat sangat
// longgar supaya tidak memblokir pemakaian nyata di dalam kantor — penjaga
// SESUNGGUHNYA ada di pengecekan JARAK ke titik kantor di bawah, karena
// buffer toleransi dari akurasi tetap dibatasi (BUFFER_AKURASI_MAKS) apa pun
// nilai akurasi yang dilaporkan device, jadi tidak bisa "dicurangi" dengan
// melaporkan akurasi yang buruk.
const BUFFER_AKURASI_MAKS = 60; // meter — toleransi tambahan dari radius kantor untuk imprecisi GPS wajar

async function processScan({ pesertaId, token, lat, lng, accuracy, curigaPalsu }) {
  const db = await getReadyDb();
  const peserta = await findPesertaById(pesertaId);
  if (!peserta) return { ok: false, message: 'Peserta tidak ditemukan. Coba login ulang.' };

  // 1. Validasi token QR yang sedang tampil di layar admin.
  const sessionCol = db.collection('session');
  const activeSession = await sessionCol.findOne({ _id: SESSION_ID });
  if (!activeSession || !token || activeSession.token !== token) {
    return { ok: false, message: 'Kode QR tidak dikenali. Pastikan mengarah ke layar admin yang benar.' };
  }
  if (activeSession.expiresAt <= Date.now()) {
    return { ok: false, message: 'Kode QR sudah kedaluwarsa. Coba scan ulang.' };
  }

  // 2. Validasi lokasi GPS terhadap titik kantor.
  const settingsDoc = await db.collection('settings').findOne({ _id: SETTINGS_ID });
  if (
    typeof lat !== 'number' ||
    typeof lng !== 'number' ||
    ...
    return {
      ok: false,
      message: 'Lokasi tidak terdeteksi. Aktifkan GPS dan izinkan akses lokasi di browser, lalu coba lagi.'
    };
  }
  if (typeof accuracy === 'number' && accuracy > MAKS_AKURASI_GPS) {
    return {
      ok: false,
      message: `Sinyal GPS terlalu lemah (akurasi ±${Math.round(accuracy)}m). Coba pindah ke tempat terbuka lalu ulangi.`
    };
  }
  const jarak = haversineMeters(lat, lng, settingsDoc.officeLat, settingsDoc.officeLng);
  const bufferAkurasi = Math.min(typeof accuracy === 'number' ? accuracy : 0, BUFFER_AKURASI_MAKS);
  const radiusEfektif = (settingsDoc.officeRadius || 0) + bufferAkurasi;
  if (jarak > radiusEfektif) {
    return {
      ok: false,
      message: `Kamu berada ${Math.round(jarak)}m dari lokasi kantor — di luar radius yang diizinkan (${settingsDoc.officeRadius}m).`
    };
  }

  const lokasiMasukInfo = { lat, lng, accuracy: accuracy ?? null, jarak: Math.round(jarak), curigaPalsu: !!curigaPalsu };

  // 3. Lanjut proses absen seperti biasa (masuk / pulang).
  const attendanceCol = db.collection('attendance');
  const tanggal = todayKey();
  const now = new Date();
  ...
  if (!existing) {
    ...
    return {
      ...
      waktu,
      status: statusMasuk,
      message: `Absen masuk tercatat · ${waktu} · ${statusMasuk} · ${Math.round(jarak)}m dari kantor`
    };
  }
  ...
  if (!existing.jamPulang) {
    await attendanceCol.updateOne(
      { pesertaId, tanggal },
      { $set: { jamPulang: waktu, pulangManual: false, lokasiPulang: lokasiMasukInfo } }
    );
...
async function simpanAlasanTerlambat({ pesertaId, alasan }) {
  const db = await getReadyDb();
  if (!pesertaId) return { ok: false, message: 'pesertaId wajib diisi.' };
  const tanggal = todayKey();
  const attendanceCol = db.collection('attendance');
  const existing = await attendanceCol.findOne({ pesertaId, tanggal });
  if (!existing || existing.sumber !== 'scan') {
    return { ok: false, message: 'Data absen masuk hari ini tidak ditemukan.' };
  }
  if (existing.statusMasuk !== 'Terlambat') {
    return { ok: false, message: 'Alasan hanya berlaku untuk absen yang tercatat terlambat.' };
  }
  const alasanBersih = String(alasan || '').trim().slice(0, 300);
  if (!alasanBersih) return { ok: false, message: 'Alasan tidak boleh kosong.' };
  await attendanceCol.updateOne({ pesertaId, tanggal }, { $set: { alasanTerlambat: alasanBersih } });
  return { ok: true };
}
...
async function getTodayLog(tanggal) {
  const db = await getReadyDb();
  const tk = /^\d{4}-\d{2}-\d{2}$/.test(tanggal || '') ? tanggal : todayKey();
  const log = await db
    .collection('attendance')
    .find({ tanggal: tk }, { projection: { _id: 0 } })
    .toArray();
  return { tanggal: tk, log };
}
